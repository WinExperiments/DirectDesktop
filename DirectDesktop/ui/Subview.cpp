...
namespace DirectDesktop
{
    ...
    LRESULT CALLBACK TopLevelWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
            ...
            for (int num = 0; num < yV->num; num++)
            {
                ...
                if (!g_touchmode)
                {
                    ...
                    if (peIcon)
                    {
                        ...
                        peIcon->SetX((rcItem.right - peIcon->GetWidth()) / 2);
                ...
                else
                {
                    ...
                    if (!g_touchmode)
                        peItemCount->SetAssociatedColor(0xFF000000 | ((groupcolor == 0) ? g_isColorized ? (iconColorID == 0) ? g_pctx->theme ?
                            RGB(64, 64, 64) : RGB(224, 224, 224) : g_pColors->crPalette[iconColorID] : g_pColors->crPalette[1] : g_pColors->crPalette[groupcolor]));
                }
            }
            break;
        }
        case WM_USER + 7:
        {
            ...
            if (yV->peOptionalTarget1 && yV->peOptionalTarget1->GetParent())
            {
                ...
                GTRANS_DESC transDesc[1];
                TransitionStoryboardInfo tsbInfo = {};
                TriggerFade(dirtitle, transDesc, 0, 0.0f, 0.2f, 0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 1.0f, true, false, false);
                ScheduleGadgetTransitions_DWMCheck(0, ARRAYSIZE(transDesc), transDesc, dirtitle->GetDisplayNode(), &tsbInfo);
                yV->peOptionalTarget1->DestroyAll(true);
                yV->peOptionalTarget1->Destroy(true);
            }
            delete yV;
    ...
    DWORD WINAPI subfastin(LPVOID lpParam)
    {
        ...
        if (!pplvi)
        {
            SendMessageW(subviewwnd->GetHWND(), WM_USER + 7, NULL, (LPARAM)yV);
            UnInitThread();
            return 0;
        }
        COLORREF colorPickerPalette[8] =
        {
            (iconColorID == 1) ? g_pColors->ImmersiveColor : IconColorizationColor, g_pColors->ImmersiveColor,
        ...
        for (int num = 0; num < yV->num; num++)
        {
            ...
            if ((*l_pm)[num]->GetFlags() & LVIF_REFRESH)
            {
                ...
                if (!((*l_pm)[num]->GetFlags() & LVIF_TEXTONLY))
                {
                    ...
                    if (IconElement)
                    {
                        colorIdx = IconElement->GetGroupColor();
                        yValueEx* yV3 = new yValueEx{ static_cast<int>(colorPickerPalette[colorIdx]),
        ...
        for (int num = 0; num < yV->num; num++)
        {
            ...
            if ((*l_pm)[num]->GetFlags() & LVIF_REFRESH)
            {
                ...
                if (di)
                {
                    ...
                    else CreateTextBitmap(capturedBitmap, (*l_pm)[num]->GetSimpleFilename().c_str(), yV->fl1, textm.tmHeight * textlines, DT_CENTER | DT_END_ELLIPSIS, g_touchmode, NULL);
                    delete yV2;
                    if (g_touchmode)
                    {
                        if (g_isGlass)
    ...
    void HidePopupCore(bool WinDInvoked, bool fNoRefresh)
    {
        ...
        for (int i = 0; i < pm.size(); i++)
        {
            ...
            else pm[i]->SetOpenDirState(LVIODS_PINNED);
        }
        if (g_issettingsopen && g_pctx->atleastonesetting)
        {
            ...
            ddnb->CreateBanner(DDNT_SUCCESS, pszTitle, nullptr, 3, nullptr);
        }
    ...
    void ShowDirAsGroup(LVItem** pplvi)
    {
        ...
        if (lviCount > 0)
        {
            ...
            RECT rcItem{};
            GetGadgetRect((*subpm)[0]->GetDisplayNode(), &rcItem, 0x4);
            DWORD animThread2;
            yValueEx* yV = new yValueEx{ lviCount, static_cast<float>(rcItem.right), NULL, subpm, PendingContainer, (Element*)pplvi };
        ...
        else
        {
            ...
            if (iconElement->GetGroupColor() >= 1)
                emptygraphic->SetAssociatedColor(g_pColors->crPalette[iconElement->GetGroupColor()]);
            CSafeElementPtr<Element> dirtitle;
    ...
    void ResetDesktopSize(Element* elem, Event* iev)
    {
        if (iev->uidType == TouchButton::Click)
        {
            ...
            if (EnsureRegValueExists(HKEY_CURRENT_USER, L"Software\\DirectDesktop", DesktopLayoutWithSize))
            {
                ...
                DDNotificationBanner* ddnb = new DDNotificationBanner();
                ddnb->CreateBanner(DDNT_SUCCESS, nullptr, clearMsg, 3, nullptr);
            }
        }
    }
