...
namespace DirectDesktop
{
	...
	LRESULT CALLBACK SubclassWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
		...
		case WM_USER + 9: {
			...
			CSafeElementPtr<TouchScrollViewer> groupdirlist; groupdirlist.Assign(regElem<TouchScrollViewer*>(L"groupdirlist", yV->peOptionalTarget1->GetParent()));
		...
		case WM_USER + 17: {
			...
			if (ppt.x < 16 * g_flScaleFactor) dragToPrev++; else dragToPrev = 0;
			if (ppt.x > dimensions.right - 16 * g_flScaleFactor) dragToNext++; else dragToNext = 0;
